...
Riportiamo un esempio di dato per una partita, troncato dove la struttura di dati è ripetitiva e dove l'informazione è ridondante, nel listato \ref{lst:data}.
...
\item[slave.GameActivity]\hfill\\
    ...
    \item[SlaveGameFragment (fig. \ref{fig:slave_game1}, \ref{fig:slave_game2} e \ref{fig:slave_game3})]\hfill\\
        contiene i nastri trasportatori e i \textit{tiles} (vedi sezione \ref{subsec:libgdx}). Istanzia i nastri trasportatori e lo \code{SlavePresenter} (che, come sul \textit{master}, istanzia lo \code{SlaveController}) in base alla modalità di gioco corrente.
...
\begin{description}
    ...
    \item[i Conveyor] sono i nastri trasportatori, su cui vengono posati i \textit{tile}. Essi possono essere di diversi tipi. Troviamo infatti \code{MovingConveyor} (figura \ref{fig:movingconveyor}), \code{SizeConveyor} (figura \ref{fig:sizeconveyor}) e \code{TowerConveyor} (figura \ref{fig:towerconveyor}).\\
